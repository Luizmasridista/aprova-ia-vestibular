...
const exerciseResults = (exerciseResultsData.data || []) as ExerciseResult[];
      ...
      const monthlyGoalsData = await monthlyGoalsService.getUserMonthlyGoals(user.id);
      
      const monthlyGoals = {
        studyHours: { current: 0, target: personalizedGoals.monthlyStudyHours },
        eventsCompleted: { current: completedEvents, target: personalizedGoals.monthlyEvents },
